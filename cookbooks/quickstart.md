...
class CacheFactory
{
    ...
    public function create(array $config): object
    {
        ...
        return match ($driver) {
            ...
            'memcached' => $this->createMemcachedCache($config),
    ...
    private function createMemcachedCache(array $config): MemcachedCache
    {
        $this->validateMemcachedConfig($config);
    ...
    private function validateRedisConfig(array $config): void
    {
        $required = ['host'];

        foreach ($required as $key) {
            if (!isset($config[$key])) {
                throw new InvalidArgumentException("Redis configuration missing required key: {$key}");
            }
        }
    }

    private function validateMemcachedConfig(array $config): void
    {
        if (!isset($config['servers']) || !is_array($config['servers'])) {
            throw new InvalidArgumentException('Memcached configuration missing servers array');
        }
...
class RedisCache
{
    ...
    public function set(string $key, mixed $value, ?int $ttl = null): bool
    {
        $this->logger->debug('Redis set', ['key' => $key, 'ttl' => $ttl]);
    ...
    public function delete(string $key): bool
    {
        $this->logger->debug('Redis delete', ['key' => $key]);
    ...
    public function clear(): bool
    {
        $this->logger->info('Redis flush database');
...
class MemcachedCache
{
    ...
    private function connect(): void
    {
        $persistentId = $this->persistent ? 'app_cache' : null;
        $this->memcached = new Memcached($persistentId);

        if (!$this->persistent || !count($this->memcached->getServerList())) {
    ...
    public function get(string $key, mixed $default = null): mixed
    {
        $this->logger->debug('Memcached get', ['key' => $key]);
    ...
    public function set(string $key, mixed $value, ?int $ttl = null): bool
    {
        $this->logger->debug('Memcached set', ['key' => $key, 'ttl' => $ttl]);
    ...
    public function delete(string $key): bool
    {
        $this->logger->debug('Memcached delete', ['key' => $key]);
    ...
    public function clear(): bool
    {
        $this->logger->info('Memcached flush');
...
class CacheServiceProvider extends ServiceProvider
{
    public function register(): void
    {
        ...
        $this->app->singleton(CacheFactory::class, function (Application $app) {
        ...
        $this->app->singleton('app.cache', function (Application $app) {
        ...
        $this->app->alias('app.cache', CacheManager::class);
    }
...
'aliases' => [
    // ... other aliases
    'AppCache' => App\Infrastructure\Cache\Facade\Cache::class,
],
```
...
class RateLimiter
{
    ...
    {
        $connection = $this->cache->connection('redis');
    ...
    {
        $this->cache->connection('redis')->delete($key);
    }
}
```
...
class FailoverCache
{
    ...
    {
        try {
            return $this->manager->connection('redis')->get($key, $default);
        } catch (\Exception $e) {
            logger()->warning('Redis failed, trying backup', ['error' => $e->getMessage()]);
            return $this->manager->connection('backup')->get($key, $default);
        }
    }
    ...
    public function set(string $key, mixed $value, ?int $ttl = null): bool
    {
        ...
        try {
            $this->manager->connection('redis')->set($key, $value, $ttl);
        } catch (\Exception $e) {
        ...
        try {
            $this->manager->connection('backup')->set($key, $value, $ttl);
        } catch (\Exception $e) {
