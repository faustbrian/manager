...
$same = $manager->connection('fedex'); // Returns same instance
```
...
$freshFedex = $manager->reconnect('fedex');
```
...
$fedexConfig = $manager->getConnectionConfig('fedex');

// Returns: ['name' => 'fedex', 'driver' => 'fedex', 'api_key' => '...', ...]
```
...
// ['fedex' => FedexClient, 'ups' => UpsClient]
...
class BookingServiceManager implements ManagerInterface
{
    ...
    private function createConnection(array $config): object
    {
        ...
        return match ($driver) {
            'fedex' => new FedexService($config),
            'ups' => new UpsService($config),
...
$manager->reconnect('fedex');

// Cleanup
$manager->disconnect('ups');
```
...
$manager->setDefaultConnection('fedex');
$service = $manager->connection();
...
{
    $first = $manager->connection('fedex');
    $second = $manager->connection('fedex');
...
{
    $original = $manager->connection('fedex');
    $reconnected = $manager->reconnect('fedex');
...
public function test_default_connection_behavior(): void
{
    ...
    $explicit = $manager->connection('fedex');
